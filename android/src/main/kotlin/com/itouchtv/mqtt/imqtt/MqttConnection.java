...
import com.itouchtv.mqtt.imqtt.db.MessageStore.StoredMessage;
...
class MqttConnection implements MqttCallbackExtended {
    ...
    private com.itouchtv.mqtt.imqtt.AlarmPingSender alarmPingSender = null;
    ...
    public void connect(MqttConnectOptions options, String invocationContext, String activityToken) {
        ...
        try {
            ...
            if (myClient != null) {//如果已经创建过MqttAsyncClient，也即之前就调用过本connect()方法
                ...
                } else if (!disconnected) {//当前已处于长连接，不再重新建立连接，直接提示连接成功
                    doAfterConnectSuccess(resultBundle);
                ...
                alarmPingSender = new com.itouchtv.mqtt.imqtt.AlarmPingSender(service);//用于发送心跳包
    ...
    /**
     * 成功建立连接后，将之前已经接收到本地但还未发送给应用层的消息继续下发。如果cleanSession为true，在这之前就会将这些消息清除
    ...
    private void deliverBacklog() {
        if (service.messageStore == null) return;
    ...
    public void messageArrived(String topic, MqttMessage message)
        ...
        String messageId = "";
        if (service.messageStore != null) {
            messageId = service.messageStore.storeArrived(clientHandle, topic, message);
        }
